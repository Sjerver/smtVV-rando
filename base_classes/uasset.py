...
#From http://www.mrob.com/pub/comp/crc-all.html
CRC32TABLE = [
...
class UAsset:
    ...
    '''
    Calculates the hash stored in the uasset for the given name using the given encoding.
            Parameters:
                    name(String): the name to hash
                    encoding(Boolean): whether the string is a utf-16 string or not
    TODO: Figure out if this actually works for utf-16 strings
    '''
    def calculateNameHash(self, name: str, encoding):
        ...
        hash_value = 0
        encoder = 'ascii'
        if encoding: #if utf-16 encoding should be used instead
            encoder = 'utf-16-le'
        upperName = name.upper()
       
        for char in upperName:
            if encoding: #utf-16 encoding is direct
                bytes = char.encode("utf-16-le")
            else: #ascii encoding uses only uppercase
                bytes = char.upper().encode('ascii')
            for raw_byte in bytes: 
                hash_value = ((hash_value >> 8) & 0x00FFFFFF) ^ CRCTABLE_DEPRECATED[(hash_value ^ raw_byte) & 0x000000FF]
        hashPart2 = 0xFFFFFFFF

        for chara in name: #utf-16 and ascii are the same for this
            char = ord(chara)
            hashPart2 = (hashPart2 >> 8) ^ CRC32TABLE[(hashPart2 ^ char) & 0xFF]
            char >>= 8
            hashPart2 = (hashPart2 >> 8) ^ CRC32TABLE[(hashPart2 ^ char) & 0xFF]
            char >>= 8
            hashPart2 = (hashPart2 >> 8) ^ CRC32TABLE[(hashPart2 ^ char) & 0xFF]
        ...
        #hashPart2 = zlib.crc32(name.encode(encoder), hashPart2) & 0xFFFFFFFF
        hashPart2 = ~hashPart2 & 0xFFFFFFFF
        #print( str((hash_value & 0xFFFF)) + " " + str(((hashPart2 & 0xFFFF) << 16)))
        endHash = ((hash_value & 0xFFFF) | ((hashPart2 & 0xFFFF) << 16)) & 0xFFFFFFFF
        return endHash
    ...
    def updateImportMaps(self):
        tempDict = {}
        for key,importEntry in self.importMap.items(): #record the updated names
        ...
        for key in list(self.reverseImportMap.keys()): #Remove the old names that have now been updated
            if key not in self.nameList:
    ...
    '''
    Updates the serial sizes and offsets of exports that are affected by a change in byte count at the uexpOffset.
            Parameters:
                    uexpOffset(Integer): offset at which changes are made
                    byteDifference(Integer): amount of bytes that have been added/removed
    '''
